...
func (t *task) buildSpec() osacv1alpha1.VolumeSpec {
	...
	if t.volume.GetSpec().HasPvcRef() {
		spec.PVCRef = &osacv1alpha1.PVCReferenceType{
			Name:      t.volume.GetSpec().GetPvcRef().GetName(),
			Namespace: t.volume.GetSpec().GetPvcRef().GetNamespace(),
			Cluster:   t.volume.GetSpec().GetPvcRef().GetCluster(),
		}
	}
	return spec
}
