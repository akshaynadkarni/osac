...
var _ = Describe("buildSpec", func() {
	...
	It("includes PVC reference when present", func() {
		...
		Expect(spec.PVCRef).ToNot(BeNil())
		Expect(spec.PVCRef.Name).To(Equal("my-pvc"))
		Expect(spec.PVCRef.Namespace).To(Equal("my-ns"))
		Expect(spec.PVCRef.Cluster).To(Equal("cluster-1"))
	})
